...
async def async_setup(hass: HomeAssistant, config: dict) -> bool:
    """Set up the Subscription Helper component."""
    ...
    async def handle_update_options(call: ServiceCall) -> None:
        ...
        if "config_entry_id" in call.data:
            ...
            raise ServiceValidationError(
                "Either config_entry_id or entity_id must be provided"
            )
        ...
        if config_entry.domain != DOMAIN:
            raise ServiceValidationError(
                f"Config entry {config_entry_id} is not a Subscription Helper"
            )
